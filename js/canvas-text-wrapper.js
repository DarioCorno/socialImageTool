  ...
  function CanvasTextWrapper(canvas, text, options) {
    ...
    function drawText() {
    	...
    	if(opts.shadow) {
    		...
    		context.shadowBlur = context.font.split("px")[0] / 3;
    		context.shadowOffsetX = 0;
    		context.shadowOffsetY = 0;
    	}
    	var oldPosX = textPos.x;
    	var oldPosY = textPos.y;
    	if(opts.spacing != 0) 
    		textPos.y -= parseInt(opts.spacing * (lines.length / 2));
    	var oldFillStyle = context.fillStyle;
		var skipLineOnMatch = multiNewLineDelimiter + ' ';
		for (var i = 0; i < lines.length; i++) {
			textPos.y = parseInt(textPos.y) + lineHeight + opts.spacing;
			if (lines[i] !== skipLineOnMatch) {
				if (opts.strokeText) {
					context.lineWidth = fontSize / 5;
					context.lineJoin = 'miter';
					context.miterLimit = 2;
					context.strokeStyle = opts.strokeCol;
					context.strokeText(lines[i], textPos.x + opts.offsetx, textPos.y + opts.offsety);
				}

				context.fillText(lines[i], textPos.x + opts.offsetx, textPos.y + opts.offsety);
			}
		}
